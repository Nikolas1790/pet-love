...
export const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  const maxPageNumberLimit = 5;
  const pageIncrement = 3; // Количество страниц для смещения
  ...
  const handlePrevBtn = () => {
    onPageChange(Math.max(currentPage - pageIncrement, 1));
  };

  const handleNextBtn = () => {
    onPageChange(Math.min(currentPage + pageIncrement, totalPages));
  };
